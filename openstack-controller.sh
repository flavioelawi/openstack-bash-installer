...
if [ $password_exists == 1 ]; then
	echo "A file with the passwords already exists, check your config!"
		...
		echo "rabbit_pass:$rabbit_pass" > openstack_pwd.txt
...
function get_passwords_from_file() {
if [ -e openstack_pwd.txt ]; then
password_exists=1
while read line
	do
	$(cut --delimiter=: openstack_pwd.txt -f 1)=$(cut --delimiter=: openstack_pwd.txt -f 2)
done < "openstack_pwd.txt"
else password_exists=0
